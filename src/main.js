...
var area = d3
  ...
  .curve(d3.curveBasis)
  .x(function (d, i) {
    return x(d3.isoParse(d.data.key));
  })
  .y0(function (d) {
    return y(d[0]);
  })
...
function rollupTo30Mins(data) {
  ...
  var entries = d3
    ...
    .key(function (d) {
      console.log(
        dateFormat(Math.round(new Date(d.date).getTime() / coeff) * coeff)
      );
      return dateFormat(Math.round(new Date(d.date).getTime() / coeff) * coeff);
    })
    .rollup(function (a) {
      var obj = {};

      fuelTechIds.forEach(function (id) {
        obj[id] = d3.mean(a, function (d) {
          return d[id];
        });
      });
...
function getData() {
  fetch("https://data.opennem.org.au/power/nem.json")
    ...
    .then(function (jsonData) {
      ...
      y.domain([0, 33000]);
      z.domain(keys);
      stack.keys(keys).value(function value(d, key) {
        return d.value[key];
      });
      ...
      var valueWrapper = tooltip.append("g").attr("class", "value-wrapper");
      ...
      layer
        ...
        .on("mouseout", function (d, i) {
          d3.select(".title").style("opacity", "1");
          svg
            .selectAll(".layer")
            .transition()
            .duration(100)
        ...
        .on("mousemove", function (d) {
          ...
          d3.select(".tooltip .date").text(
            formatTime(d3.isoParse(xDateData.date))
          );
          d3.select(".tooltip .value").text(
            fuelTechLabels[hoverFuelTech] +
              ": " +
              xDateData[hoverFuelTech] +
              " MW"
          );
